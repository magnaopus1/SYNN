...
// NewGrantApprovalProcess initializes a new grant approval process.
func NewGrantApprovalProcess(ledgerInstance *ledger.Ledger, consensusEngine *common.SynnergyConsensus, authorityNodes []common.NodeType) *BusinessPersonalGrantApprovalProcess {
	return &BusinessPersonalGrantApprovalProcess{
		Ledger:            ledgerInstance,
		Consensus:         consensusEngine,
		Proposals:         make(map[string]*BusinessPersonalGrantProposalApproval),
...
// StartApprovalProcess initiates the approval process for a grant proposal.
func (ap *BusinessPersonalGrantApprovalProcess) StartApprovalProcess(proposal *BusinessPersonalGrantProposal) error {
	ap.mutex.Lock()
	...
	proposalApproval := &BusinessPersonalGrantProposalApproval{
		Proposal:      proposal,
...
func (ap *BusinessPersonalGrantApprovalProcess) SubmitPublicVote(businessName string, voterAddress string, confirm bool) error {
	ap.mutex.Lock()
	defer ap.mutex.Unlock()
...
func (ap *BusinessPersonalGrantApprovalProcess) SubmitAuthorityNodeVote(businessName, nodeID string, confirm bool) error {
	ap.mutex.Lock()
	defer ap.mutex.Unlock()
