...
// NewEcosystemGrantApprovalProcess initializes a new grant approval process for ecosystem grants.
func NewEcosystemGrantApprovalProcess(ledgerInstance *ledger.Ledger, consensusEngine *common.SynnergyConsensus, authorityNodes []common.NodeType) *EcosystemGrantApprovalProcess {
	return &EcosystemGrantApprovalProcess{
		Ledger:            ledgerInstance,
		Consensus:         consensusEngine,
		Proposals:         make(map[string]*EcosystemGrantProposalApproval),
...
// StartApprovalProcess initiates the approval process for an ecosystem grant proposal.
func (ap *EcosystemGrantApprovalProcess) StartApprovalProcess(proposal *EcosystemGrantProposal) error {
	ap.mutex.Lock()
	...
	fmt.Printf("Ecosystem grant proposal for %s has entered Stage 1 (Public Voting).\n", proposal.BusinessName)
	return nil
}

// SubmitPublicVote allows the community to vote on an ecosystem grant proposal during Stage 1.
func (ap *EcosystemGrantApprovalProcess) SubmitPublicVote(businessName string, voterAddress string, confirm bool) error {
	ap.mutex.Lock()
	defer ap.mutex.Unlock()
	...
	if time.Since(proposalApproval.VoteStartTime) > ap.PublicVotePeriod {
		...
		if acceptVotes > rejectVotes && len(proposalApproval.PublicVotes) > 0 {
			...
			fmt.Printf("Ecosystem grant proposal for %s has moved to Stage 2 (Authority Node Confirmation).\n", businessName)
		} else {
			proposalApproval.Stage = -1 // Rejected by public vote.
			fmt.Printf("Ecosystem grant proposal for %s has been rejected by public vote.\n", businessName)
		}
	}
...
func (ap *EcosystemGrantApprovalProcess) SubmitAuthorityNodeVote(businessName, nodeID string, confirm bool) error {
	ap.mutex.Lock()
	defer ap.mutex.Unlock()
	...
	if proposalApproval.ConfirmationCount >= 5 {
		...
		fmt.Printf("Ecosystem grant proposal for %s has been confirmed by authority nodes.\n", businessName)
	} else if proposalApproval.RejectionCount >= 5 {
		proposalApproval.Stage = -1 // Proposal rejected.
		fmt.Printf("Ecosystem grant proposal for %s has been rejected by authority nodes.\n", businessName)
	}
...
func (ap *EcosystemGrantApprovalProcess) MonitorAuthorityNodeTimeout() {
	ap.mutex.Lock()
	defer ap.mutex.Unlock()
