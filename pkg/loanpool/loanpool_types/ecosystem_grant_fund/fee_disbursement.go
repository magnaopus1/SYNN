...
// NewEcosystemGrantDisbursementManager initializes a new disbursement manager for the Ecosystem Grant Fund.
func NewEcosystemGrantDisbursementManager(ledgerInstance *ledger.Ledger, consensusEngine *common.SynnergyConsensus, initialBalance float64) *EcosystemGrantDisbursementManager {
...
// RequestDisbursement is called when the proposer wishes to disburse funds after confirmation for the Ecosystem Grant Fund.
func (fdm *EcosystemGrantDisbursementManager) RequestDisbursement(proposalID string, proposerWallet string, amount float64) error {
	fdm.mutex.Lock()
	...
	queueEntry := &EcosystemGrantDisbursementQueueEntry{
...
func (fdm *EcosystemGrantDisbursementManager) ProcessDisbursementQueue() {
	fdm.mutex.Lock()
	defer fdm.mutex.Unlock()

	newQueue := []*EcosystemGrantDisbursementQueueEntry{}
...
// GetFundBalance returns the current balance of the Ecosystem Grant Fund.
func (fdm *EcosystemGrantDisbursementManager) GetFundBalance() float64 {
	fdm.mutex.Lock()
	defer fdm.mutex.Unlock()
...
func (fdm *EcosystemGrantDisbursementManager) GetDisbursementQueue() []*EcosystemGrantDisbursementQueueEntry {
	fdm.mutex.Lock()
	defer fdm.mutex.Unlock()
