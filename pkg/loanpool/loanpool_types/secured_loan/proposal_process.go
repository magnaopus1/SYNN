...
// NewProposalManager initializes a new manager for handling secured loan proposals.
func NewProposalManager(
	...
	collateralMgr *common.CollateralManager,
	...
	return &common.ProposalManager{
		...
		Proposals:         make(map[string]*common.SecuredLoanProposal),
	}
}

// StartProposalProcess begins the loan application process with the applicant details.
func (pm *common.ProposalManager) StartProposalProcess(applicantName, applicantID, walletAddress string) (*common.SecuredLoanProposal, error) {
	pm.mutex.Lock()
	...
	proposal := &common.SecuredLoanProposal{
		LoanID:              loanID,
	...
	fmt.Printf("Loan application process started for applicant %s (Loan ID: %s).\n", applicantName, loanID)
	return proposal, nil
}
...
// SubmitCollateral handles the collateral submission for the loan.
func (pm *common.ProposalManager) SubmitCollateral(loanID, collateralType string, collateralValue float64, collateralProof, iouLegalDocument []byte) error {
	pm.mutex.Lock()
	defer pm.mutex.Unlock()
	...
	// Submit collateral
	collateralSubmission, err := pm.CollateralMgr.SubmitCollateral(loanID, proposal.WalletAddress, collateralType, collateralValue, collateralProof, iouLegalDocument)
	if err != nil {
		return fmt.Errorf("collateral submission failed: %v", err)
	}

	// Update proposal with collateral status
	proposal.CollateralStatus = collateralSubmission.ApprovalStatus
	proposal.ApprovalStage = "Collateral"
	proposal.LastUpdated = time.Now()

	// Store the updated proposal in the ledger
	err = pm.Ledger.UpdateProposal(proposal)
	if err != nil {
		return fmt.Errorf("failed to update proposal after collateral submission: %v", err)
	}

	fmt.Printf("Collateral submitted for Loan ID: %s. Status: %s.\n", loanID, collateralSubmission.ApprovalStatus)
	return nil
}
...
func (pm *common.ProposalManager) FinalizeProposal(loanID string) error {
	...
	if proposal.CreditScore == 0 || proposal.AffordabilityStatus == "" || proposal.CollateralStatus == "" || !proposal.TermsCustomization {
