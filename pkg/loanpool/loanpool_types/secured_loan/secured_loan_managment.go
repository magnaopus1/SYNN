...
// NewSecuredLoanManagement initializes a new instance for managing secured loans.
func NewSecuredLoanManagement(ledgerInstance *ledger.Ledger, encryptionService *encryption.Encryption, consensusEngine *common.SynnergyConsensus, networkManager *common.NetworkManager, authorityNodes map[string]*common.AuthorityNode) *common.SecuredLoanManagement {
	return &common.SecuredLoanManagement{
...
func (lm *common.SecuredLoanManagement) UpdateAuthorityWallet(nodeID, newWalletAddress string) error {
	lm.mutex.Lock()
...
func (lm *common.SecuredLoanManagement) UpdateBorrowerDetails(loanID, name, email, contact, walletAddress string) error {
	lm.mutex.Lock()
	defer lm.mutex.Unlock()
...
func (lm *common.SecuredLoanManagement) RequestTermChange(loanID, newTerms string) error {
	lm.mutex.Lock()
	defer lm.mutex.Unlock()
...
func (lm *common.SecuredLoanManagement) HandleAuthorityDecision(loanID, nodeID string, approved bool) error {
	lm.mutex.Lock()
	defer lm.mutex.Unlock()
...
func (lm *common.SecuredLoanManagement) distributeTermChangeRequestToNodes(request *common.BorrowerTermChangeRequest) {
...
func (lm *common.SecuredLoanManagement) sendTermChangeRequestToNode(node *common.AuthorityNode, request *common.BorrowerTermChangeRequest) error {
...
func (lm *common.SecuredLoanManagement) finalizeTermChange(loanID string, request *common.BorrowerTermChangeRequest) {
...
func (lm *common.SecuredLoanManagement) selectRandomAuthorityNodes(count int) map[string]*common.AuthorityNode {
...
func (lm *common.SecuredLoanManagement) selectRandomAuthorityNodeExcluding(exclude map[string]*common.AuthorityNode) *common.AuthorityNode {
	for {
		node := lm.AuthorityNodes[common.GenerateRandomNodeID()]
