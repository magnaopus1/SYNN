...
// NewUnsecuredLoanApprovalProcess initializes the approval process for unsecured loan proposals.
func NewUnsecuredLoanApprovalProcess(ledgerInstance *ledger.Ledger, nodes []*common.AuthorityNode, encryptionService *encryption.Encryption, networkManager *common.NetworkManager) *common.UnsecuredLoanApprovalProcess {
	return &common.UnsecuredLoanApprovalProcess{
...
// StartApprovalProcess begins the approval process for a new unsecured loan proposal.
func (p *common.UnsecuredLoanApprovalProcess) StartApprovalProcess(proposal *common.UnsecuredLoanProposal) error {
	p.mutex.Lock()
...
func (p *common.UnsecuredLoanApprovalProcess) HandleNodeDecision(proposalID, nodeID string, confirmed bool, interestRate float64) error {
	p.mutex.Lock()
	defer p.mutex.Unlock()
...
func (p *common.UnsecuredLoanApprovalProcess) RequeueProposals() {
	p.mutex.Lock()
	defer p.mutex.Unlock()
...
func (p *common.UnsecuredLoanApprovalProcess) distributeProposalToNodes(proposal *common.ActiveProposal) {
...
func (p *common.UnsecuredLoanApprovalProcess) sendProposalToNode(node *common.AuthorityNode, proposal *common.UnsecuredLoanProposal) error {
...
func (p *common.UnsecuredLoanApprovalProcess) finalizeProposal(proposal *common.ActiveProposal) {
...
func (p *common.UnsecuredLoanApprovalProcess) calculateAverageInterest(rates []float64) float64 {
	var total float64
...
func (p *common.UnsecuredLoanApprovalProcess) selectRandomNodes(count int) map[string]*common.AuthorityNode {
...
func (p *common.UnsecuredLoanApprovalProcess) selectRandomNodeExcluding(exclude map[string]*common.AuthorityNode) *common.AuthorityNode {
	rand.Seed(time.Now().UnixNano())
...
func (p *common.UnsecuredLoanApprovalProcess) getActiveProposalByID(proposalID string) (*common.ActiveProposal, bool) {
