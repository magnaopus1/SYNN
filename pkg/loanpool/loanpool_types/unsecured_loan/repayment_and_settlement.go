...
func (rm *common.SecuredLoanRepaymentManager) SetupRepaymentPlan(loanID string, totalAmount float64, interestRate float64, proposerWallet string, repaymentDates []time.Time, authorityWallets []string, defaultContact string) error {
	rm.mutex.Lock()
...
func (rm *common.SecuredLoanRepaymentManager) HandleDefault(loanID string) error {
	...
	if time.Since(repaymentDetails.NextPaymentDue) > rm.DefaultThreshold {
		...
		// Notify the authority nodes via email for default notification.
		rm.notifyAuthorityNodesForDefault(repaymentDetails.DefaultContact)
		...
		fmt.Printf("Loan %s has defaulted. Default notification sent.\n", loanID)
		return nil
	}
...
// notifyAuthorityNodesForDefault sends a notification to authority nodes for default on loan.
func (rm *common.SecuredLoanRepaymentManager) notifyAuthorityNodesForDefault(defaultContact string) {
	// Implement the notification mechanism here (e.g., email).
	fmt.Printf("Default notification sent to %s due to loan default.\n", defaultContact)
}
